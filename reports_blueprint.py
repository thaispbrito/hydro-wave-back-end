...
def create_report():
    try:
        ...
        cursor.execute("""
                        ...
                        RETURNING id
                        """,
                       (author_id, title, reported_at, water_source, water_feature, location_lat, location_long, observation, condition, status, created_at, updated_at, image_url)
                       )
        ...
        cursor.execute("""SELECT h.id, 
                            ...
                            r.updated_at,
                            r.image_url,
                            r_report.username AS author_username
                        FROM reports r
...
def update_report(report_id):
    try:
        ...
        cursor.execute("UPDATE reports SET title = %s, reported_at = %s, water_source = %s, water_feature = %s, location_lat = %s, location_long = %s, observation = %s, condition = %s, status = %s, created_at = %s, updated_at = %s, image_url = %s WHERE reports.id = %s RETURNING *",
                       (title, reported_at, water_source, water_feature, location_lat, location_long, observation, condition, status, created_at, updated_at, final_image_url, report_id))
        report_id = cursor.fetchone()["id"]
